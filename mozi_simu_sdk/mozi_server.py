...
class MoziServer:
    ...
    def start_mozi_server(self):
        ...
        时间：2020.05.04
        """
        if self.platform == 'windows' and self.server_ip == '127.0.0.1':
            # 判断墨子是否已经启动
            ...
            if not is_mozi_server_started:
                ...
                print("%s：仿真推演方服务端已启动" % (datetime.datetime.now()))
        ...
        while not is_connected:
            ...
            if connect_cout > 60:
                break
            print("%s：还没连接上仿真推演服务器,再等1秒" % (datetime.datetime.now()))
            time.sleep(1)

        if is_connected:
            if self.agent_key_event_file:
                self.write_key_event_string_to_file('成功连接仿真推演服务器！')
            print("%s：成功连接仿真推演服务器！" % (datetime.datetime.now()))
        else:
    ...
    def clean_mission_by_side_name(self, side_name):
        """
        功能：清除指定推演方内置任务
        参数：side_name， str, 推演方名称
        返回：无
        作者：张志高
        单位：北京华戍防务技术有限公司
        时间：2022-8-30
        """
        return self.send_and_recv(f"ScenEdit_ClearMission('{side_name}')")
