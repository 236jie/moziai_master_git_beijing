...
class CSide:
    ...
    def delete_group(self, group_name, remove_child='false'):
        ...
        参数：
            group_name {str: 编组名称}
            remove_child 是否删除编组内子单元 {str: 'true' - 是，就是删除了单元   'false' - 否}
        返回：'lua执行成功' 或 '脚本执行出错'
        作者：张志高
        单位：北京华戍防务技术有限公司
        时间：2021-7-20
    ...
    def add_unit(self, unit_type, name, dbid, latitude, longitude, heading=None):
        """
        功能：添加单元  # 朝向和高度设置不起作用
        限制：专项赛禁用
        参数：
        ...
        cmd = ("HS_LUA_AddUnit({side = '%s', guid = '%s', type = '%s', name = '%s', dbid = %s, latitude = %s, "
               "longitude = %s})"
               % (self.strName, guid, unit_type, name, dbid, latitude, longitude))
    ...
    def add_submarine(self, name, dbid, latitude, longitude, heading=None):
        """
        功能：添加潜艇  # 朝向和高度设置不起作用
        限制：专项赛禁用
        参数：
        ...
        """
        guid = self.situation.generate_guid()
        cmd = ("HS_LUA_AddUnit({type = 'SUB', name = '%s', guid = '%s', dbid = %s, "
               "side = '%s', latitude=%s, longitude=%s})"
               % (name, guid, dbid, self.strName, latitude, longitude))
        rslt = self.mozi_server.send_and_recv(cmd)
        obj = None
        if rslt == 'lua执行成功':
            self.mozi_server.throw_into_pool(cmd)
    ...
    def add_ship(self, name, dbid, latitude, longitude, heading=None):
        """
        功能：添加舰船 # 朝向和高度设置不起作用
        限制：专项赛禁用
        参数：
        ...
        guid = self.situation.generate_guid()
        print(guid)
        cmd = ("HS_LUA_AddUnit({type = 'ship', name = '%s', guid = '%s', dbid = %s, "
               "side = '%s', latitude=%s, longitude=%s})"
               % (name, guid, dbid, self.strName, latitude, longitude))
        rslt = self.mozi_server.send_and_recv(cmd)
        obj = None
    ...
    def add_aircraft(self, name, dbid, loadoutid, latitude, longitude, altitude=None, heading=None):
        """
        功能：添加飞机  # 高度和朝向设置不起作用
        限制：专项赛禁用
        参数：
        ...
        """
        guid = self.situation.generate_guid()
        cmd = ("HS_LUA_AddUnit({type = 'air', name = '%s', guid = '%s', loadoutid = %s, dbid = %s, "
               "side = '%s', latitude=%s, longitude=%s})"
               % (name, guid, loadoutid, dbid, self.strName, latitude, longitude))
        rslt = self.mozi_server.send_and_recv(cmd)
        obj = None
        if rslt == 'lua执行成功':
            self.mozi_server.throw_into_pool(cmd)
    ...
    def set_network(self,send_guid, recv_guid, MsgPassProbability=None, MsgDelayMin=None, MsgDelayMax=None):
        ...
        :param recv_guid: 接收方GUID
        :param MsgPassProbability: 信息传递成功率 （0-1）之间
        :param MsgDelayMin: 信息最小延迟时间  1 标识 ，单位秒
        :param MsgDelayMax: 信息最大延迟时间，单位秒
        :return: 'lua执行成功' 或 '脚本执行出错'
        作者：赵俊义
        单位：北京华戍防务技术有限公司
        时间：2020-10-13
    ...
    def set_network_core(self, guid, air_cycle=None,  ship_cycle=None, sub_cycle=None, fac_cycle=None):
        ...
        :param air_cycle: 情报中心对飞机的下传周期, 单位秒  例：60
        :param ship_cycle: 情报中心对舰艇的下传周期, 单位秒 例：300
        :param sub_cycle: 情报中心对潜艇的下传周期, 单位秒  例：7200
        :param fac_cycle: 情报中心对地面设施的下传周期, 单位秒 例：60
        :return: 'lua执行成功' 或 '脚本执行出错'
        作者：赵俊义
        单位：北京华戍防务技术有限公司
        时间：2020-10-13
    ...
    def get_network_contact(self, guid, num):
        """
        ...
        修改： 张志高 2021-12-27
        """
        lua_script = f'ReturnObj(NET_GetContacts("{guid}", {num}))'
        contact_dict = self.__convert_network_contact_str_to_dict(self.mozi_server.send_and_recv(lua_script))
        return contact_dict

    def __convert_network_contact_str_to_dict(self, return_str):
        # 功能：将get_network_contact接口返回的字符串，生成对象列表
        if 'lua执行成功' in return_str:
            return []
        ...
        for item in return_list:
            if 'guid =' in item:
                ...
                if item.endswith(','):
                    item = item[:-1]
                kv = item.split('=')
                list2.append(kv[1].strip().replace("'", ''))
        # list3 = []
        # for item in return_list:
        #     if 'batchNum =' in item:
        #         item = item.strip()
        ...
        #             item = item[:-1]
        #         kv = item.split('=')
        #         list3.append(kv[1].strip().replace("'", ''))
        return_dict = {}
        for item in list2:
            if item in self.get_contacts().keys():
                return_dict[item] = self.get_contacts()[item]
            else:
                print(f"{item}没有在探测目标列表")
                # TODO 后续考虑推送模式获取到的目标不在列表中的情况
        return return_dict
