...
class CScenario:
    ...
    def set_fineness(self, detailed_gun_fire_control=None, unlimited_base_mags=None, aircraft_damage=None,
                     comms_jamming=None, comms_disruption=None, ballistic_missile=None, sim_plex_network=None):
        """
        功能：设置想定精细度
        限制：专项赛禁用
        函数类别：编辑使用函数
        参数：
            ...
            sim_plex_network 是否使用简单通信网络 {str: true - 是，false - 否}
        返回：'lua执行成功' 或 '脚本执行出错'
        作者：张志高
        单位：北京华戍防务技术有限公司
        时间：2021-8-17
        ...
        if sim_plex_network:
            update_str += f", SimPlexNetWork={sim_plex_network}"
