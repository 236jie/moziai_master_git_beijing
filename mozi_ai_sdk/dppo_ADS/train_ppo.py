...
from mozi_ai_sdk.dppo_ADS.agents.ppo_agent import PPOActor, PPOLearner
...
def start_learner():
    ...
    class Tempenv(object):
        def __init__(self):
            # 动作空间维度：1（不拦截） + 3种防空系统×2种拦截数量（战斧） + 3种防空系统×1种拦截数量（JASSM） = 1 + 6 + 3 = 10
            self.action_space = MaskDiscrete(10)
            # 观察空间维度：3种导弹系统×3个状态 + 2种导弹×3个状态 + 3个拦截状态 = 9 + 6 + 3 = 18
            self.observation_space = spaces.Tuple([spaces.Box(0.0, float('inf'), [18], dtype=np.float32),
                                                   spaces.Box(0.0, 1.0, [10], dtype=np.float32)])
