...
class Environment:
    ...
    def step(self):
        ...
        self.scenario.mozi_server.run_grpc_simulate()
    ...
    def reset(self, side_name):
        ...
        # self.create_waypoint(side_name)
        self.scenario.mozi_server.run_simulate()
        # self.step()
    ...
    def connect_mozi_server(self, ip, port):
        """
        功能：连接墨子服务器
        参数：
        返回：
        ...
        server_ip = ip
        aiPort = str(port)
        self.mozi_server = MoziServer(server_ip, aiPort, self.platform, self.scenario_name,
    ...
    def create_waypoint(self, side_name):
        for k, v in self.redside.referencepnts.items():
            if 'plan-0' in v.strName:
                start_longitude = v.dLongitude
                start_latitude = v.dLatitude
            if 'plan-30' in v.strName:
                end_longitude = v.dLongitude
                end_latitude = v.dLatitude
        delta_longitude = abs(start_longitude - end_longitude)/30
        delta_latitude = abs(start_latitude - end_latitude)/30
        for index in range(1, 30):
            temp_lon = start_longitude + delta_longitude * index
            temp_lan = start_latitude - delta_latitude * index
            self.redside.add_reference_point(f'{side_name}', 'plan-%d' % index, temp_lan, temp_lon)
