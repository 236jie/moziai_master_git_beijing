...
class WorkerSet:
    ...
    def _make_worker(
        ...
        # by dixit
        if self._remote_config['env_config']['avail_port'] and self._local_config['env_config']['avail_port']:
            self._remote_config['env_config']['avail_port'].pop(0)
            self._local_config['env_config']['avail_port'].pop(0)
        else:
            raise Exception('work_set._make_worker: no avail port!')
