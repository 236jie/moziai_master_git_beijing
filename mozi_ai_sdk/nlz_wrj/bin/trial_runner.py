...
# ray release port
ray_release_port = zmq_context.socket(zmq.PUSH)


def release_port(rel_port):
    ray_release_port.setsockopt(zmq.SNDHWM, 1)
    ray_release_port.setsockopt(zmq.RCVHWM, 1)
    ray_release_port.connect("tcp://%s:%s" % ('localhost', '5706'))
    try:
        container = client.containers.get('pydockertest_%s' % rel_port)
        container.stop()
        container.remove()
    except Exception:
        print('fail get or stop or remove pydockertest_%s container!' % rel_port)
    # port = 6265
    ray_release_port.send_pyobj(rel_port)


def request_port():
    port_requestor.connect("tcp://%s:%s" % ('localhost', '5705'))
    port_requestor.send_string("request port")
    port = port_requestor.recv_pyobj()
...
class TrialRunner:
    ...
    def step(self):
        ...
        if next_trial is not None:
            with warn_if_slow("start_trial"):
                # by dixit 2020/12/8
                next_trial.config['env_config']['avail_port'] = []
                for i in range(next_trial.config['num_workers'] + 1):
                    avail_port = request_port()
                    next_trial.config['env_config']['avail_port'].append(avail_port)
    ...
    def _execute_action(self, trial, decision):
        ...
        if decision == TrialScheduler.CONTINUE:
            ...
            # by dixit
            # kill_proc()
            for rel_port in trial.config['env_config']['avail_port']:
                release_port(rel_port)
        else:
