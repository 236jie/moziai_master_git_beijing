...
class TestStrikeMission(TestFramework):
    ...
    def test_assign_unit(self):
        ...
        for k, v in units.items():
            flag = True
            self.assertTrue(v.strGuid in [self.antisubmarine_aircraft_guid, self.antisubmarine_aircraft_2_guid])
    ...
    def test_assign_unit_as_target(self):
        ...
        mission.assign_unit_as_target(self.enemy_airplane_guid)
        self.env.step()
        self.assertTrue(self.enemy_airplane_guid in mission.get_targets().keys())
    ...
    def test_set_preplan(self):
        ...
        self.env.step()
        self.assertEqual(mission.bPrePlannedOnly, True)

        mission.set_preplan(False)
        self.env.step()
        self.assertEqual(mission.bPrePlannedOnly, False)
    ...
    def test_set_minimum_trigger(self):
        ...
        self.env.step()
        self.assertEqual(mission.m_MinimumContactStanceToTrigger, 1)
        # 非友
        mission.set_minimum_trigger(2)
        self.env.step()
        self.assertEqual(mission.m_MinimumContactStanceToTrigger, 2)
        # 敌对
        mission.set_minimum_trigger(3)
        self.env.step()
        self.assertEqual(mission.m_MinimumContactStanceToTrigger, 3)
        # 不明
        mission.set_minimum_trigger(4)
        self.env.step()
        self.assertEqual(mission.m_MinimumContactStanceToTrigger, 4)
    ...
    def test_set_min_aircrafts_required(self):
        ...
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, 0)

        mission.set_min_aircrafts_required(1)
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -97)

        mission.set_min_aircrafts_required(2)
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -96)

        mission.set_min_aircrafts_required(3)
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -95)

        mission.set_min_aircrafts_required(4)
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -94)

        mission.set_min_aircrafts_required(6)
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -93)

        mission.set_min_aircrafts_required(8)
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -92)

        mission.set_min_aircrafts_required(12)
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -91)

        mission.set_min_aircrafts_required('all')
        self.env.step()
        self.assertEqual(mission.m_MinAircraftReq_Strikers, -99)
    ...
    def test_set_radar_usage(self):
        ...
        self.env.step()
        self.assertEqual(mission.m_RadarBehaviour, 2)
        # 整个飞行计划遵循任务电磁管控规则
        mission.set_radar_usage(1)
        self.env.step()
        self.assertEqual(mission.m_RadarBehaviour, 1)
        # 从进入攻击航线段到武器消耗完毕状态点打开雷达
        mission.set_radar_usage(3)
        self.env.step()
        self.assertEqual(mission.m_RadarBehaviour, 3)
    ...
    def test_set_fuel_ammo(self):
        ...
        self.env.step()
        self.assertEqual(mission.m_Bingo, 0)
        # 在最远距离上抛弃空对地弹药，以获取最大打击
        mission.set_fuel_ammo(1)
        self.env.step()
        self.assertEqual(mission.m_Bingo, 1)
        # 如果不能打击目标，则带回空对地弹药
        mission.set_fuel_ammo(2)
        self.env.step()
        self.assertEqual(mission.m_Bingo, 2)
    ...
    def test_strike_radius(self):
        ...
        self.env.step()
        self.assertEqual(mission.iMinResponseRadius, 200)
        self.assertEqual(mission.iMaxResponseRadius, 300)
    ...
    def test_set_flight_size_check(self):
        ...
        self.env.step()
        self.assertEqual(mission.bUseFlightSizeHardLimit, True)
        mission.set_flight_size_check('false')
        self.env.step()
        self.assertEqual(mission.bUseFlightSizeHardLimit, False)
    ...
    def test_set_auto_planner(self):
        ...
        self.env.step()
        self.assertEqual(mission.bUseAutoPlanner, True)
        mission.set_auto_planner('false')
        self.env.step()
        self.assertEqual(mission.bUseAutoPlanner, False)

    def test_set_strike_one_time_only(self):
        """设置打击任务是否仅限一次"""
        mission = self.red_side.get_missions_by_name('空中打击')
        mission.set_strike_one_time_only('true')
        self.env.step()
        self.assertEqual(mission.bOneTimeOnly, True)
        mission.set_strike_one_time_only('false')
        self.env.step()
        self.assertEqual(mission.bOneTimeOnly, False)
    ...
    def test_set_strike_escort_flight_size_shooter(self):
        ...
        self.env.step()
        self.assertEqual(mission.m_EscortFlightSize, 4)

    def test_set_strike_escort_response_radius(self):
        """设置打击任务护航最大威胁响应半径"""
        mission = self.red_side.get_missions_by_name('空中打击')
        mission.set_strike_escort_response_radius(200)
        self.env.step()
        self.assertEqual(mission.iEscortResponseRadius, 200)
