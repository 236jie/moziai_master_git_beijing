...
class TestFramework(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if etc.app_mode in [1, 2]:
            ...
            if not is_mozi_server_started:
                os.environ['MOZIPATH'] = etc.MOZI_PATH_2
                mozi_path = os.environ['MOZIPATH']
                mozi_server_exe_file = mozi_path + '\\' + 'MZServer.exe'
        ...
        # MK-54 轻型鱼雷 #65
        self.weapon_1_guid = '3412b534-b1b7-4b30-b6cc-d932c8281525'
        self.weapon_1 = self.red_side.get_unit_by_guid(self.weapon_1_guid)

        self.doctrine_owner = self.red_side
        # self.doctrine_owner = self.red_side.get_missions_by_name('空中打击')
    ...
    def tearDown(self):
        if etc.app_mode in [1, 2]:
            os.system('TASKKILL /F /IM Mozi.exe')
        else:
            os.system('TASKKILL /F /IM MZServer.exe')
        time.sleep(5)
        print("--------------- CASE END ----------------------------")