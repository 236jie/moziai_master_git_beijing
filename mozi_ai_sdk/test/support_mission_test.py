...
class TestSupportMission(TestFramework):
    ...
    def test_assign_unit(self):
        ...
        for k, v in units.items():
            if v.strGuid in [self.antisubmarine_aircraft_guid]:
                flag = True
    ...
    def test_set_one_time_only(self):
        ...
        self.env.step()
        self.assertEqual(mission.bOTO, True)

        mission.set_one_time_only('false')
        self.env.step()
        self.assertEqual(mission.bOTO, False)

    def test_set_loop_type(self):
        """导航类型"""
        # TODO 设置后不起作用
        mission = self.red_side.get_missions_by_name('加油任务')
        mission.set_loop_type('continuation')
        self.env.step()
        self.assertEqual(mission.iNLT, 0)
        mission.set_loop_type('onceRepeat')
        self.env.step()
        self.assertEqual(mission.iNLT, 1)
    ...
    def test_set_emcon_usage(self):
        ...
        self.env.step()
        self.assertEqual(mission.bAEOOS, True)

        mission.set_emcon_usage('false')
        self.env.step()
        self.assertEqual(mission.bAEOOS, False)

    def test_add_plan_way_to_mission(self):
        """为任务分配预设航线"""
        mission = self.red_side.get_missions_by_name('加油任务')
        self.red_side.add_plan_way(0, '出航航线-新')
        self.red_side.add_plan_way(0, '返航航线-新')
        # 出航航线
        mission.add_plan_way_to_mission(0, '出航航线-新')
        # 返航航线
        mission.add_plan_way_to_mission(2, '返航航线-新')
        self.env.step()

        sideways = self.red_side.get_sideways()
        side_way_unit_1 = common.get_obj_by_name(sideways, '出航航线-新')
        side_way_unit_2 = common.get_obj_by_name(sideways, '返航航线-新')

        self.assertEqual(mission.m_strSideRTBWayGUID, side_way_unit_2.strGuid)
        self.assertEqual(mission.m_strSideWayGUID, side_way_unit_1.strGuid)
    ...
    def test_set_throttle_submarine(self):
        ...
        self.env.step()
        self.assertEqual(mission.m_StationThrottle_Submarine, 3)
        self.assertEqual(mission.m_TransitThrottle_Submarine, 2)
    ...
    def test_set_submarine_depth(self):
        ...
        self.env.step()
        self.assertEqual(mission.strTransitDepth_Submarine, '-100')
        self.assertEqual(mission.strStationDepth_Submarine, '-50')


if __name__ == '__main__':
    TestSupportMission.main()
