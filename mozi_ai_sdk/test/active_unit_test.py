...
class TestActiveUnit(TestFramework):
    ...
    def test_plot_course(self):
        """航线规划"""
        # 反潜机1
        course_list = [(26.072826770494, 125.58281397334), (26.41034316517, 125.85757557944)]
        self.antisubmarine_aircraft.plot_course(course_list)
        self.env.step()
        way_points_info = self.antisubmarine_aircraft.get_way_points_info()
        self.assertEqual(way_points_info[0]['latitude'], 26.072826770494)
        self.assertEqual(way_points_info[0]['longitude'], 125.58281397334)
        self.assertEqual(way_points_info[1]['latitude'], 26.41034316517)
        self.assertEqual(way_points_info[1]['longitude'], 125.85757557944)
    ...
    def test_hold_position(self):
        """命令面上指定单元设置是否保持阵位"""
        self.ground_to_air_missile_squadron.hold_position('true')
        self.env.step()
        self.assertTrue(self.ground_to_air_missile_squadron.bHoldPosition)
        self.ground_to_air_missile_squadron.hold_position('false')
        self.env.step()
    ...
    def test_set_unit_heading(self):
        """设置单元朝向"""
        # TODO 设置单元朝向对舰船不起作用
        self.submarine.set_unit_heading(60)
        self.env.step()
        self.assertEqual(self.submarine.fCurrentHeading, 60)
    ...
    def test_set_throttle(self):
        ...
        self.env.step()
        # 设置单元油门
        # TODO 设置低速前，必须设置期望速度才能生效
        self.aircraft_dipping_sonar.set_desired_speed(50.1)
        ...
        self.refuel_aircraft.set_throttle(4)
        self.env.step()
        self.assertEqual(4, self.refuel_aircraft.m_CurrentThrottle)
    ...
    def test_set_desired_height_submarine(self):
        """设置期望高度"""
        # TODO C++ fDesiredAltitude态势值为整数
        # 给潜艇设置航线
        course_list = [(26.0728267704942, 125.582813973341), (26.410343165174, 125.857575579442)]
        self.submarine.plot_course(course_list)
        self.env.step()
        # 设置单元期望高度
        self.submarine.set_desired_height(-50, 'true')
        self.env.step()
        self.assertEqual(-50, self.submarine.fDesiredAltitude)
    ...
    def test_drop_active_sonobuoy(self):
        ...
        flag = False
        for k, v in weapons.items():
            if '主动声纳浮标' in v.strName and v.strName != name_1:
                self.assertTrue(v.fAltitude_AGL > -50)
                flag = True
                break
        self.assertEqual(True, flag)
    ...
    def test_drop_passive_sonobuoy(self):
        ...
        flag = False
        for k, v in weapons.items():
            if '被动声纳浮标' in v.strName and v.strName != name_1:
                self.assertTrue(v.fAltitude_AGL > -50)
                flag = True
                break
        self.assertEqual(True, flag)
    ...
    def test_allocate_weapon_auto_targeted(self):
        ...
        # TODO 不限制开火时，发射10枚
        # TODO C++ 脚本执行出错
        # Hs_AllocateWeaponAutoTargeteds('fe0bd1ae-552a-49e1-b265-2f7ab8d0b60a',{'801ea534-a57c-4d3b-ba5d-0f77e909506c','781cc773-30e3-440d-8750-1b5cddb90249'},1152,4)
        # self.ground_to_air_missile_squadron.wcsf_contact_types_unit('Hold')
    ...
    def test_update_way_point(self):
        ...
        self.antisubmarine_aircraft.plot_course(course_list)
        self.env.step()
        self.antisubmarine_aircraft.update_way_point(0, 27.072826770494, 126.58281397334)
        self.env.step()
        way_points_info = self.antisubmarine_aircraft.get_way_points_info()
        self.assertEqual(way_points_info[0]['latitude'], 27.072826770494)
        self.assertEqual(way_points_info[0]['longitude'], 126.58281397334)
