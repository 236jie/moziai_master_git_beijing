...
RED_MISSILE_INFO_MAP = {"C-400": (2104, "萨姆-21B型", 48), "HQ-9A": (1225,"HQ-9A", 32), "HQ-12": (123,"HQ-12", 24)}  # 武器DBID及理论装弹数
RED_MISSILE_AVAILABLE = {"C-400": 48, "HQ-9A": 32, "HQ-12": 24}  # HQ-12初始可用数量12，打完后剩下的12枚导弹重装载需要2分钟
...
def _manual_attack(unit, target, missile_count, weapon_dbid):
    """
    调用墨子平台的手动打击接口
...
class SDFKEnv(MultiAgentEnv):
    ...
    def __init__(self, env_config):
        ...
        self.missile_inventory = {
            'C-400': 48 * 2,  # 2个营，每营48枚
            'HQ-9A': 32 * 3,  # 3个营，每营32枚
            'HQ-12': 24 * 4  # 4个营，每营24枚（注意重装填机制）
        }
    ...
    def _execute_engagement_plan(self, engagement_plan):
        ...
        for plan in engagement_plan:
            ...
            if last_engaged_step is not None:
                ...
                if threat_score > 7.0 or target_distance < 30:  # 高威胁或很近
                    dynamic_cooldown = 0  # 高威胁时立即允许再次拦截
                elif threat_score > 5.0 or target_distance < 50:  # 中等威胁
                    dynamic_cooldown = max(0, self.base_cooldown_steps - 1)  # 缩短冷却
                elif threat_score < 3.0 and target_distance > 80:  # 低威胁且较远
                ...
                if (self.steps - last_engaged_step) < dynamic_cooldown:
                    ...
                    if threat_score >= 8.0 or target_distance < 25:
            ...
            for defense_type, missile_count in assignments.items():
                ...
                # 1. 检查该类型单元是否有足够导弹
                # if self.missile_inventory[defense_type] < missile_count:
                #     print(
                #         f"警告：{defense_type} 导弹库存不足！需要{missile_count}枚，剩余{self.missile_inventory[defense_type]}枚")
                #     missile_count = self.missile_inventory[defense_type]  # 用尽剩余导弹
                ...
                if defense_type == 'C-400' and missile_count == 1:
                    if self.steps > 100:  # 前100步不输出
                        print('执行C-400，打出1枚，得分0.1')
                    self.temp_reward += 0.1
                # 如果defense_type是HQ-9A，并且missile_count>=2, 则给予奖励0.1
                elif defense_type in ['HQ-9A', 'HQ-12'] and missile_count >= 2:
                    # print('执行其他两种武器，打出2枚，得分0.1')
                    self.temp_reward += 0.1
                ...
                # 5. 更新单元发射记录（用于追踪发射历史，装弹时间由墨子平台内部管理）
                if success:
                    unit_guid = best_unit_dict['unit'].strGuid
                    # 记录发射历史（主要用于调试和分析，实际的装弹时间由墨子平台管理）
